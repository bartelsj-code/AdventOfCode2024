...
def main():
    ...
    while len(two_identified) > 0:
        ...
        if m2_gate == None or type(m2_gate) != type(gate):
            print(gate)
            print(wire_translation_for)
            # print(f"{in_wires1} not found or wrong type in template. originally:{gate}")
            # print("problem noticed")

            # top_gates = [wire.pusher for wire in gate.inputs]
            # for top_gate in top_gates:
            #     names = [wire_translation_for[g.name] for g in top_gate.inputs]
            #     tup = (names[0], top_gate.tag, names[1])

            #     c_top = m2_gates[tup]
            #     print(f"correct top: \t\t\t {c_top}")

            #     correct_children      
            #     print(c_top.output.pullers)
            #     # print(m2)
        ...
        if gate.output.name not in wire_translation_for and m2_gate.output.name not in wire_translation_back:
